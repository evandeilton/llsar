---
title: "Estimando os parâmetros de um Modelo SAR (Spatial AutoRegressive) por máxima verossimilhança."
author: "José Lopes"
output:
...
require(rgdal)
require(sp)
m <- rgdal::readOGR("dados/shp/Limite_de_Bairros.shp", verbose = FALSE, stringsAsFactors = FALSE, 
...
A <- methods::as(rio_shape, "symmetricMatrix") %>% as.matrix

# Y: Variável resposta que se refere ao total de tiroteios registrados no período de 6 meses com final em novembro/2020.
Y <- rio_tiroteio$count
```
...
\end{aligned}$$ e como dado por definição do problema $\textbf{Y} \sim \textbf{N}_{mv} (\mu\textbf{1}, \sigma^2(\textbf{I}-\rho \textbf{A})^{-2}).$ Com isso a meta é estimar, através do método de máxima verossimilhança, os parâmetros do modelo dado com maximização através do método de Newton-Rapson. Para tanto, determina-se três funções principais:
  ...
  - (1) $l(\theta)$ que é o log da função de verossomilhança $L(\theta)$ para uma realização de uma normal multivariada com média $E(\textbf{Y}) = \mu\textbf{1}$ e variância $Var(\textbf{Y}) = \sigma^2(\textbf{I}-\rho \textbf{A})^{-2}$
  - (2) vetor gradiente de $l(\theta)$ e 
  - (3) matriz hessiana de $l(\theta)$, onde $\theta = (\beta^{\top}, \sigma^2, \rho)$.
...
```{r, fig.pos="h", fig.align='center', fig.height=3, fig.width=10, fig.cap="Perfis de verossimilhança"}
par(mfrow = c(1,3))
plot(fit1_profile)
```
...
```{r, echo=T, eval=F}
a2 <- fit_newton(lvero = sar_reg_lvero_analitico, gr = sar_reg_gradiente_analitico, 
                 hess = sar_reg_hessiano_analitico,
                 formula = count ~ loginhabitants, dados = rio_tiroteio, A = A, verbose = F)
fit2 <- fit_quasi_newton(a2)
fit2_profile <- profile(fit1)
```
...
```{r, fig.pos="h", fig.align='center', fig.height=6, fig.width=10, fig.cap="Perfis de verossimilhança"}
par(mfrow = c(2,2))
plot(fit2_profile)
```
...
```{r, eval=F, echo=T}
predict_simples     <- sar_reg_predict(fit1) %>% rename("pred_s" = pred)
predict_covariaveis <- sar_reg_predict(fit2) %>% rename("pred_c" = pred)
```


```{r}
simples <- sar_reg_predict(fit1) %>% rename("pred_s" = pred)
covaria <- sar_reg_predict(fit2) %>% rename("pred_c" = pred)
predis  <- rio_tiroteio %>% 
...
```{r, fig.pos="h", fig.align='center', fig.height=4, fig.width=10, fig.cap="Perfis de verossimilhança"}
